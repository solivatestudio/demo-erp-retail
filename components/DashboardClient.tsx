...
const attention = [
  ...
  { icon: ArrowRight, title: "2 delivery belum selesai", note: "Menunggu konfirmasi penerima", href: "/delivery", tone: "neutral" },
];
...
export default function DashboardClient() {
  ...
  return <div className="dashboard-page owner-dashboard">
    ...
    <section className="dashboard-content-grid">
      ...
      <Card className="outlet-chart"><CardHeader><CardTitle>Performa outlet</CardTitle><CardDescription>Kontribusi omzet bulan ini</CardDescription></CardHeader><CardContent><div className="donut-wrap"><div className="donut"/><div className="outlet-total"><strong>Rp482,6 jt</strong><span>Total omzet</span></div></div><div className="outlet-legend">{outlets.map(o=><div key={o.name}><i style={{background:o.color}}/><span>{o.name}</span><strong>{o.value}%</strong><span className="outlet-progress"><i style={{width:`${o.value}%`,background:o.color}}/></span></div>)}</div></CardContent></Card>
      ...
      <section className="mobile-shortcuts"><h2>Aksi cepat</h2><div><Link href="/pos">Buka POS <ArrowRight size={14}/></Link><Link href="/purchases">Buat pembelian <ArrowRight size={14}/></Link><Link href="/inventory/stock">Cek inventory <ArrowRight size={14}/></Link></div></section>
    </section>
  </div>;
}
